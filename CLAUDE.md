...
snJava — a Chrome Extension (Manifest V3) that assists ServiceNow development: **F1 Design Spec Generator** (record → HTML/PDF/Word spec in the MFEC light theme), **F2 Script Tester** (3 layers: static lint → prod-guarded background-script run (bgrun) → guarded real execution), **F3 XML Mover** (copy/paste record unload XML across instances).
...
- **Script Tester Layer 2 runs on the instance, prod-guarded.** The former in-extension mock sandbox was removed (superseded by `bgrun`); the tester script now runs as a real background script (`sys.scripts.do`) gated by the prod guard, so instance safety is the prod guard's job (next rule), not a mock.
...
- **F2 tester**: `lint.ts` (Layer 1 anti-pattern engine, comment/string-aware) → Layer 2 runs the tester script as a prod-guarded background script (bgrun) via `sn-rest.ts` → guarded create/delete via `sn-rest.ts` (Layer 3).
